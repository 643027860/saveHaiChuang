  ...
  <div class="manage-item hc-user-direction">
    ...
    <div class="manage-item-component">
      ...
      <div class="manage-item-component-content">
        ...
        <el-button type="text" size="small" class="pc" @click="handlerEdit">编辑</el-button>
        <el-button type="text" size="small" @click="deleteGroup">删除</el-button>
      </div>
    </div>
    <!--  方向编辑弹窗  -->
...
<script lang="ts">
  ...
  import {GetAllGroup, GetAllDirById, UpdateDir, InsertDir, DeleteDir} from '../../../api/types';
  import {myMessage} from '../../../utils/message';

  export default defineComponent({
    name: 'UserDirectionDetail',
    // 路由导航，如果没传入方向 id 则重定向方向列表
    beforeRouteEnter(to, from, next) {
      if (to.query.directionId) {
        next()
      } else {
        next({name: 'user-direction'})
      }
    },
    setup() {
      const router = useRouter() // 声明路由
      const route = useRoute() // 声明路由
      const directionId = route.query.directionId // 方向 id
      ...
      const getDetail = async() => {
        const result = await GetAllDirById({directionId})
        if (result.status === 0) {
      ...
      // 删除组别
      const deleteGroup = async (directorId: string) => {
        const result = await DeleteDir({directorId})
        if (result.status === 0) {
          myMessage('success', result.msg, 1200)
          getGroups() // 重新获取组别信息
        } else {
      ...
      const goDetail = (groupId: string) => {
        router.push({name: 'user-direction', query: {groupId}})
      }
      // 重置表单
